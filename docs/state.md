...
|---|---|---|---|
| `libvmaf_vulkan.h` not installed under prefix → FFmpeg `--enable-libvmaf-vulkan` silently drops the filter (lawrence repro 2026-04-28 19:27) | this PR | — | `meson install --destdir /tmp/x` produces `/tmp/x/usr/local/include/libvmaf/libvmaf_vulkan.h` post-fix (was missing); FFmpeg `configure --enable-libvmaf-vulkan` then passes the `check_pkg_config libvmaf_vulkan ... libvmaf/libvmaf_vulkan.h vmaf_vulkan_state_init_external` probe and the `libvmaf_vulkan` filter actually builds |
