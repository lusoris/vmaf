...
| ARM NEON | auto-detected on aarch64 | yes | `--cpumask` | stable |
...
| HIP (AMD) | `-Denable_hip=true` | no | experimental | experimental |
...
- [../usage/cli.md](../usage/cli.md) — `--no_cuda` / `--no_sycl` / `--sycl_device` /
  `--cpumask` / `--gpumask` flags.
