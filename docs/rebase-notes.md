...
- **Twin-update gaps**:
  - **AVX-512**: closed 2026-05-08 — `calculate_c_values_row_avx512` landed in `libvmaf/src/feature/x86/cambi_avx512.c` (16-lane gather + mmask predicate; bit-identical to scalar / AVX-2 at full IEEE-754 precision). Dispatch flips to `calculate_c_values_avx512` when AVX-512BW + DQ + VL are present. See ADR-0328's "Status update 2026-05-08" appendix and `test_cambi_simd` parity gate.
  - **NEON**: closed 2026-05-08 — `calculate_c_values_row_neon` landed in `libvmaf/src/feature/arm64/cambi_neon.c` (4-lane vectorised arithmetic + scalar histogram gathers; NEON has no general-purpose vector gather). Dispatch in `calculate_c_values_neon` switched to the new row stage. Bit-exactness gated by `test_cambi_simd` on aarch64 CI.
