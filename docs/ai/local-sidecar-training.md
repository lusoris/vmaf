...
> **Status**: scaffold — public API + persistence layout + closed-form
> online-ridge fit ship in this PR.
> Operator-facing CLI wiring, drift-warning surface, and opt-in upload
> to a community pool are tracked as follow-ups in
> [ADR-0325](../adr/0325-local-sidecar-training.md) §Future work.
...
[ADR-0325](../adr/0325-local-sidecar-training.md) §Privacy posture).
...
[`docs/ai/inference.md`](inference.md). For the scaffold there is
nothing to allowlist.

## Validation metrics

The sidecar is validated by five contract tests under
...
The suite is sub-second and CPU-only.

The drift-detection signal (`SidecarModel.recent_residual_rms`) is
exposed for a future PR that thresholds on it; the scaffold does
not surface it to the operator.
...
scaffold PR**. It is tracked as a follow-up in
[ADR-0325](../adr/0325-local-sidecar-training.md) §Future work and
