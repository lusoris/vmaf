...
| [ADR-0144](0144-port-netflix-41d42c9e-vif-edge-mirror.md) | Port Netflix upstream `41d42c9e` + `bc744aa3` paired (VIF edge-mirror bugfix `+1 → +2` in 5 sites + ~40-assertion `places=4 → places=3` golden loosening). Accepted final-VMAF drift ~1e-3 as sub-model-floor noise; no retrain. Establishes standing interpretation: rule #1 "never modify Netflix golden assertions" addresses fork drift, not upstream-authored test updates the fork must track (ADR-0142 precedent, ADR-0143 repeat). ADR-0141 applied: `convolution_internal.h` and `vif_tools.c` are zero clang-tidy warnings; four upstream functions NOLINT'd for T7-5 sweep. | Accepted | upstream-port, correctness, vif, testing, golden-gate |
