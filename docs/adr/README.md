...
| [ADR-0123](0123-cuda-post-cubin-load-regression-32b115df.md) | Focused investigation of the post-cubin-load CUDA crash introduced by upstream `32b115df` (experimental `VMAF_BATCH_THREADING` / `VMAF_PICTURE_POOL` threading modes) — bisect, reproduce against GPU runner, decide revert vs fix vs off-by-default gate | Proposed | cuda, investigation, upstream-sync |
