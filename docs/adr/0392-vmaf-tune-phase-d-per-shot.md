# ADR-0392: `vmaf-tune` Phase D — per-shot CRF tuning scaffold

- **Status**: Accepted (scaffold only; native per-codec emission and
  Phase B predicate wiring deferred)
- **Date**: 2026-05-03
- **Deciders**: Lusoris
...
- **Phase B target-VMAF bisect** in flight (PR #347) — its predicate
  shape ``(shot, target_vmaf, encoder) -> (crf, predicted_vmaf)`` is
  the hook the per-shot loop calls per shot.
...
ADR ships that layer as a **scaffold** with stable public API, two
explicitly-pluggable integration seams, and mocked smoke coverage.

The scaffold is gated by three follow-up dependencies we deliberately
do not ship in the same PR:

1. Phase B's bisect must land as code so the default predicate stops
   returning the codec adapter's default CRF.
2. Per-codec native per-shot emission (x264 `--qpfile`, x265
   `--zones`, SVT-AV1 segment tables) must land per-codec — the
   scaffold uses per-segment encode plus concat-demuxer, which is
   portable but loses GOP-alignment efficiency.
3. A held-out per-shot validation corpus must exist so we can claim
   numerical wins; without it, this PR ships zero quality claims.

## Decision

We will ship `tools/vmaf-tune/src/vmaftune/per_shot.py` as the
Phase D scaffold with the following public surface:
...
- ``tune_per_shot(shots, *, target_vmaf, encoder, predicate=None)``
  — drives the predicate per shot. Default predicate returns the
  codec adapter's ``quality_default`` so the scaffold round-trips
...
- **Positive**:
  ...
  - Mocked smoke coverage: all 16 new tests pass without ``ffmpeg``,
...
- **Negative**:
  - Ships zero numerical quality wins; until Phase B and the codec
    emission follow-ups land, the default predicate just stamps the
    codec's default CRF on every shot.
  - Per-segment + concat encoding loses keyframe alignment with the
    encoder's natural GOP — efficiency penalty vs native
    ``--qpfile``/``--zones``. Acceptable for a scaffold; the codec
    PRs replace it per-codec.
...
- **Neutral / follow-ups**:
  - Phase B PR (#347) wires its bisect as the production
    ``predicate`` argument once it lands. No API change required.
