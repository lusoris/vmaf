...
- [ADR-0284](../docs/adr/0284-fr-regressor-v2-codec-schema-expansion.md) — `CODEC_VOCAB` v2 (16 slots). The active vocabulary now covers software (x264, x265, libsvtav1, libaom) + NVENC + QSV + AMF + VideoToolbox + a trailing `reserved` slot. `CODEC_VOCAB_VERSION = 2`. The v1 6-slot vocabulary is preserved as `CODEC_VOCAB_V1` for back-compat with `fr_regressor_v1.onnx` callers; never alias the two names. `tools/vmaf-tune/tests/test_codec_one_hot.py` pins each (label → index) pair as a regression gate.
