...
def test_cache_batches_index_writes(tmp_path, monkeypatch):
    """Verify that 100 cache puts result in at most 2 file writes."""
    c = TuneCache(tmp_path / "cache")

    # Patch Path.write_text to count writes to __index__.json.
    original_write_text = Path.write_text
    write_counts = {"index": 0}

    def counting_write_text(self, *args, **kwargs):
        if self.name == "__index__.json":
    ...
    # Without flush, writes should be 0 because puts only mark dirty.
    # The dirty flag prevents writes until flush() is called.
    # Now flush — should write once.
    c.flush()

    total_writes = write_counts["index"]

    # After the 100 puts and one flush, we expect at most 2 writes:
    # - evict_lru may trigger if size_bytes is exceeded (but default is 10 GB)
    # - flush() writes once at the end
    # Since we didn't exceed the size cap with 100 × 100 byte blobs,
    # we expect exactly 1 write from the flush.
    assert total_writes <= 2, f"Expected at most 2 index writes, got {total_writes}"
