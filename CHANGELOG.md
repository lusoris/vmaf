...
- **`speed_qa` real SpEED-QA implementation** (`libvmaf/src/feature/speed_qa.c`):
  replaces the placeholder scaffold with a working spatial and temporal
  ...
  output = spatial + temporal per frame. Self-contained (no float dependency).
  ADR-0253, docs/metrics/speed_qa.md.
- **`libvmaf/test/test_speed_qa.c`**: five smoke tests covering registration,
  vtable, flat/noise entropy ordering, and temporal positivity.
...
- **HIP batch-3 — `float_moment_hip` and `float_ssim_hip` real kernels (ADR-0375)**: promotes two HIP feature extractors from `-ENOSYS` stubs to real `#ifdef HAVE_HIPCC` dual-path consumers. Device kernels: `float_moment/moment_score.hip` (four uint64 atomic accumulators [ref1st, dis1st, ref2nd, dis2nd], 64-lane warp reduction via two `uint32` `__shfl_down` shuffles, host divides by w×h, emits four `float_moment_*` features) and `float_ssim/ssim_score.hip` (two-pass 11-tap separable Gaussian — horiz pass writes five intermediate float buffers over (W-10)×H, vert+combine pass computes per-block float partial sums over (W-10)×(H-10); `SSIM_WARPS_PER_BLOCK=2` for 64-lane GCN/RDNA vs CUDA's 4; host accumulates in `double`, emits `float_ssim`; v1 `scale=1` only). Both host TUs follow the `float_psnr_hip.c` dual-path pattern: `hipMalloc` device staging, `hipMemcpy2DAsync` HtoD, `hipModuleLaunchKernel`, `hipEventRecord` + `hipMemcpyAsync` readback; free helpers defined outside `HAVE_HIPCC` with internal guards. `moment_score` and `ssim_score` added to `hip_kernel_sources` in `src/meson.build`. HIP real-kernel count: 6 of 11 extractors. Requires `enable_hip=true` + `enable_hipcc=true`; CPU-only builds preserve the `-ENOSYS` scaffold posture.
