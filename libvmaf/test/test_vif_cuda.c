...
/*
 * CUDA parity test for float_vif_cuda.
 *
 * Goals (ADR-0214 / ADR-0138 / ADR-0139):
 *   1. Verify vmaf_fex_float_vif_cuda is discoverable by name.
 *   2. Init the CUDA extractor on a synthetic 256×144 YUV420P 8-bpc frame.
 *   3. Assert that the primary score key ("VMAF_feature_vif_scale0_score")
...
/* ------------------------------------------------------------------ */
static char *test_vif_cuda_setup(void)
{
    ...
    if (err) {
        (void)fprintf(stderr,
                      ...
                      "no GPU available — skipping float_vif_cuda tests\n",
                      err);
        cuda_init_failed = 1;
        cuda_state = NULL;
        return NULL;
...
/* ------------------------------------------------------------------ */
/* Test 1: registration — float_vif_cuda is discoverable.              */
/* ------------------------------------------------------------------ */
static char *test_vif_cuda_registration(void)
{
    VmafFeatureExtractor *fex = vmaf_get_feature_extractor_by_name("float_vif_cuda");
    mu_assert("vmaf_fex_float_vif_cuda should be findable by name", fex != NULL);
    if (fex) {
        mu_assert("fex name should be float_vif_cuda", strcmp(fex->name, "float_vif_cuda") == 0);
...
/* ------------------------------------------------------------------ */
static char *test_vif_cuda_smoke(void)
{
    if (cuda_init_failed) {
        (void)fprintf(stderr, "  [SKIP] test_vif_cuda_smoke (no GPU)\n");
        return NULL;
    }
    ...
    mu_assert("vmaf_use_feature(float_vif_cuda) should succeed", err == 0);
    if (err) {
        (void)vmaf_close(vmaf);
        return NULL;
    }
    ...
    double score = -1.0;
    err = vmaf_feature_score_at_index(vmaf, "VMAF_feature_vif_scale0_score", &score, 0);
    mu_assert("vif_scale0_score retrieval should succeed", err == 0);
    mu_assert("vif_scale0_score should be finite and non-negative",
...
/* Teardown.                                                            */
/* ------------------------------------------------------------------ */
static char *test_vif_cuda_teardown(void)
{
    if (cuda_state != NULL) {
...
char *run_tests(void)
{
    mu_run_test(test_vif_cuda_setup);
    mu_run_test(test_vif_cuda_registration);
    mu_run_test(test_vif_cuda_smoke);
    mu_run_test(test_vif_cuda_teardown);
    return NULL;
}
