...
"""Smoke test for the shipped ``vmaf_tiny_v3`` ONNX model.

Mirrors ``test_vmaf_tiny_v2.py`` (ADR-0244 / ADR-0275). Verifies, end
to end, that:

  1. The registry entry for ``vmaf_tiny_v3`` exists, points to a
...
ONNX_PATH = TINY / "vmaf_tiny_v3.onnx"
SIDECAR_PATH = TINY / "vmaf_tiny_v3.json"
...
class VmafTinyV3Smoke(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_registry_entry_present_and_digest_matches(self) -> None:
        ...
        entry = next(
            (m for m in reg.get("models", []) if m.get("id") == "vmaf_tiny_v3"),
            None,
        )
        self.assertIsNotNone(entry, "registry.json missing vmaf_tiny_v3 entry")
        self.assertEqual(entry["onnx"], "vmaf_tiny_v3.onnx")
    ...
    def test_sidecar_well_formed(self) -> None:
        ...
        self.assertEqual(side["id"], "vmaf_tiny_v3")
        self.assertEqual(side["kind"], "fr")
        self.assertEqual(side["onnx"], "vmaf_tiny_v3.onnx")
...
if __name__ == "__main__":
    # Allow running directly via `python3 test_vmaf_tiny_v3.py`.
    os.chdir(REPO_ROOT)
