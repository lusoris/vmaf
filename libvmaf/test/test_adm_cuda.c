...
/*
 * CUDA parity test for adm_cuda (integer_adm_cuda).
 *
 * Goals (ADR-0214 / ADR-0138 / ADR-0139):
 *   1. Verify vmaf_fex_integer_adm_cuda is discoverable by name.
 *   2. Init the CUDA extractor on a synthetic 256×144 YUV420P 8-bpc frame.
 *   3. Assert that the primary score key ("VMAF_integer_feature_adm2_score")
 *      is finite and non-negative.
 *   4. Skip cleanly when no CUDA device is present (no CUDA driver / no GPU).
 *
 * This is a smoke + sanity test, not a cross-backend numerical gate.
...
/* ------------------------------------------------------------------ */
static char *test_adm_cuda_setup(void)
{
    ...
    if (err) {
        (void)fprintf(stderr,
                      ...
                      "no GPU available — skipping adm_cuda tests\n",
                      err);
        cuda_init_failed = 1;
        cuda_state = NULL;
        return NULL;
...
/* ------------------------------------------------------------------ */
/* Test 1: registration — adm_cuda is discoverable.                    */
/* ------------------------------------------------------------------ */
static char *test_adm_cuda_registration(void)
{
    VmafFeatureExtractor *fex = vmaf_get_feature_extractor_by_name("adm_cuda");
    mu_assert("vmaf_fex_integer_adm_cuda should be findable by name", fex != NULL);
    if (fex) {
        mu_assert("fex name should be adm_cuda", strcmp(fex->name, "adm_cuda") == 0);
...
/* ------------------------------------------------------------------ */
static char *test_adm_cuda_smoke(void)
{
    if (cuda_init_failed) {
        (void)fprintf(stderr, "  [SKIP] test_adm_cuda_smoke (no GPU)\n");
        return NULL;
    }
    ...
    mu_assert("vmaf_use_feature(adm_cuda) should succeed", err == 0);
    if (err) {
        (void)vmaf_close(vmaf);
        return NULL;
    }
    ...
    /* Fill both planes with a mid-grey flat value. */
    ...
    double score = -1.0;
    err = vmaf_feature_score_at_index(vmaf, "VMAF_integer_feature_adm2_score", &score, 0);
    mu_assert("adm2_score retrieval should succeed", err == 0);
    mu_assert("adm2_score should be finite and non-negative", isfinite(score) && score >= 0.0);
...
/* Teardown.                                                            */
/* ------------------------------------------------------------------ */
static char *test_adm_cuda_teardown(void)
{
    if (cuda_state != NULL) {
...
char *run_tests(void)
{
    mu_run_test(test_adm_cuda_setup);
    mu_run_test(test_adm_cuda_registration);
    mu_run_test(test_adm_cuda_smoke);
    mu_run_test(test_adm_cuda_teardown);
    return NULL;
}
