                           ...
                           int w, int h, int src_stride, int dst_stride)
{
    ...
    for (i = 0; i < (h + 1) / 2; ++i) {
        ...
        for (j = 0; j + 16 <= w; j += 16) {
             ...
             * Use mul+add (not FMA) to match scalar rounding behavior. */
            __m512 lo_acc = _mm512_add_ps(_mm512_mul_ps(fl0, s0), _mm512_mul_ps(fl1, s1));
            ...
            __m512 hi_acc = _mm512_add_ps(_mm512_mul_ps(fh0, s0), _mm512_mul_ps(fh1, s1));
         ...
         * scalar for left boundary (j=0 reflection) and right tail. */
        {
            ...
            for (; j + 16 <= half_w && 2 * j + 32 < w; j += 16) {
                ...
                __m512 ba = _mm512_add_ps(_mm512_mul_ps(fl0, tap0), _mm512_mul_ps(fl1, tap1));
                ...
                __m512 bv = _mm512_add_ps(_mm512_mul_ps(fh0, tap0), _mm512_mul_ps(fh1, tap1));
                ...
                __m512 bh = _mm512_add_ps(_mm512_mul_ps(fl0, tap0), _mm512_mul_ps(fl1, tap1));
                ...
                __m512 bd = _mm512_add_ps(_mm512_mul_ps(fh0, tap0), _mm512_mul_ps(fh1, tap1));
