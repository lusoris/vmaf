...
static inline void
ssim_accumulate_block_avx512(const float *ref_mu, const float *cmp_mu, const float *ref_sigma_sqd,
                             const float *cmp_sigma_sqd, const float *sigma_both, int i, __m512 vC1,
                             __m512 vC2, __m512 vC3, __m512 vzero, float C1, float C2,
                             double *local_ssim, double *local_l, double *local_c, double *local_s)
{
                            ...
                            double *s_sum)
{
    ...
    for (; i + 16 <= n; i += 16) {
        ssim_accumulate_block_avx512(ref_mu, cmp_mu, ref_sigma_sqd, cmp_sigma_sqd, sigma_both, i,
                                     vC1, vC2, vC3, vzero, C1, C2, &local_ssim, &local_l, &local_c,
                                     &local_s);
    }
    for (; i < n; i++) {
