...
adm_csf_den_scale_line_kernel(const cuda_adm_dwt_band_t src, int h, int top, int bottom, int left,
                              int right, int src_stride, uint64_t *accum)
{
...
#pragma unroll
    ...
    if ((threadIdx.x % VMAF_CUDA_THREADS_PER_WARP) == 0) {
        uint32_t shift_accum =
            (uint32_t)__float2uint_ru(__log2f((bottom - top) * (right - left)) - 20);
        shift_accum = shift_accum > 0 ? shift_accum : 0;
        int32_t add_shift_accum = shift_accum > 0 ? (1 << (shift_accum - 1)) : 0;
        atomicAdd((uint64_cu *)&accum[band - 1], (temp_value + add_shift_accum) >> shift_accum);
    }
}

template <int val_per_thread, int cta_size>
__device__ __forceinline__ void
adm_csf_den_s123_line_kernel(const cuda_i4_adm_dwt_band_t src, int h, int top, int bottom, int left,
                             int right, int src_stride, uint32_t add_shift_sq, uint32_t shift_sq,
                             uint64_t *accum)
{
...
#define ADM_CSF_SCALE_LINE(val_per_thread, cta_size)                                               \
    ...
    {                                                                                              \
        adm_csf_den_scale_line_kernel<val_per_thread, cta_size>(src, h, top, bottom, left, right,  \
                                                                src_stride, accum);                \
    }
