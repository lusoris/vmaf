 ...
 *
 *  float_moment feature extractor on the HIP backend.
 *  File is named `integer_moment_hip.c` to mirror the CUDA and SYCL
 *  naming convention:
 *    - CUDA twin: `libvmaf/src/feature/cuda/integer_moment_cuda.c`
 *    - SYCL twin: `libvmaf/src/feature/sycl/integer_moment_sycl.cpp`
 *  The "integer" prefix reflects integer YUV pixel accumulation even
 *  though the emitted metrics are named `float_moment_*`.
 *
...
#define INTEGER_MOMENT_HIP_COUNTERS 4u

/* Block dimensions for the moment kernel (mirrors CUDA twin). */
#define INTEGER_MOMENT_HIP_BX 16u
#define INTEGER_MOMENT_HIP_BY 16u

/* ------------------------------------------------------------------ */
/* HIP-to-errno translation                                            */
/* ------------------------------------------------------------------ */

static int integer_moment_hip_rc(hipError_t rc)
{
    if (rc == hipSuccess)
        return 0;
    switch (rc) {
...
typedef struct IntegerMomentStateHip {
 ...
 */
static int integer_moment_hip_module_load(IntegerMomentStateHip *s, unsigned bpc, unsigned w,
                                          unsigned h)
{
    hipError_t hip_rc = hipModuleLoadData(&s->module, integer_moment_score_hsaco);
    if (hip_rc != hipSuccess)
        return integer_moment_hip_rc(hip_rc);

    hip_rc =
        hipModuleGetFunction(&s->funcbpc8, s->module, "calculate_integer_moment_hip_kernel_8bpc");
    if (hip_rc != hipSuccess) {
    ...
    hip_rc =
        hipModuleGetFunction(&s->funcbpc16, s->module, "calculate_integer_moment_hip_kernel_16bpc");
    if (hip_rc != hipSuccess) {
        (void)hipModuleUnload(s->module);
        s->module = NULL;
        return integer_moment_hip_rc(hip_rc);
 ...
 */
static int integer_moment_hip_launch(IntegerMomentStateHip *s, VmafPicture *ref_pic,
                                     VmafPicture *dist_pic)
{
    ...
    hipError_t hip_rc = hipMemsetAsync(s->rb.device, 0,
                                       (size_t)INTEGER_MOMENT_HIP_COUNTERS * sizeof(uint64_t), str);
    if (hip_rc != hipSuccess)
        return integer_moment_hip_rc(hip_rc);
    ...
    const unsigned gx = (s->frame_w + INTEGER_MOMENT_HIP_BX - 1u) / INTEGER_MOMENT_HIP_BX;
    const unsigned gy = (s->frame_h + INTEGER_MOMENT_HIP_BY - 1u) / INTEGER_MOMENT_HIP_BY;
    void *args[] = {
        &s->ref_in,   (void *)&row_w, &s->dis_in,  (void *)&row_w,
        s->rb.device, &s->frame_w,    &s->frame_h,
    };
    hipFunction_t func = (s->bpc == 8u) ? s->funcbpc8 : s->funcbpc16;
    /* 16bpc kernel has the same 7-arg signature as 8bpc -- the kernel reads
     * raw uint16_t pixels regardless of bpc depth; host already sized
     * ref_in/dis_in at 2 bytes per pixel. */
    hip_rc = hipModuleLaunchKernel(func, gx, gy, 1u, INTEGER_MOMENT_HIP_BX, INTEGER_MOMENT_HIP_BY,
                                   1u, 0, str, args, NULL);
    if (hip_rc != hipSuccess)
        return integer_moment_hip_rc(hip_rc);
    ...
    hip_rc = hipMemcpyAsync(s->rb.host_pinned, s->rb.device,
                            (size_t)INTEGER_MOMENT_HIP_COUNTERS * sizeof(uint64_t),
...
static void integer_moment_hip_module_free(IntegerMomentStateHip *s)
{
#ifdef HAVE_HIPCC
    if (s->dis_in != NULL) {
...
{
    (void)pix_fmt;
    IntegerMomentStateHip *s = fex->priv;
...
#ifdef HAVE_HIPCC
    err = integer_moment_hip_module_load(s, bpc, w, h);
#else
    err = -ENOSYS;
#endif
    if (err != 0)
...
fail_after_mod:
    integer_moment_hip_module_free(s);
fail_after_rb:
...
#ifdef HAVE_HIPCC
    if (s->dis_in != NULL) {
        int e = integer_moment_hip_rc(hipFree(s->dis_in));
        s->dis_in = NULL;
        if (rc == 0)
            rc = e;
    }
    if (s->ref_in != NULL) {
        int e = integer_moment_hip_rc(hipFree(s->ref_in));
        s->ref_in = NULL;
        if (rc == 0)
            rc = e;
    }
    if (s->module != NULL) {
        int e = integer_moment_hip_rc(hipModuleUnload(s->module));
        s->module = NULL;
        if (rc == 0)
            rc = e;
    }
...
// NOLINTNEXTLINE(misc-use-internal-linkage) -- load-bearing registry symbol; same as CUDA/SYCL twins
VmafFeatureExtractor vmaf_fex_float_moment_hip = {
    ...
    .close = close_fex_hip,
    .options = options,
    .priv_size = sizeof(IntegerMomentStateHip),
