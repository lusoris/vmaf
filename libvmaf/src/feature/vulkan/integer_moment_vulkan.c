 ...
 *
 *  integer_moment feature extractor on the Vulkan backend.
 *  Direct port of libvmaf/src/feature/cuda/integer_moment_cuda.c
 *  (T7-23 / ADR-0182, GPU long-tail batch 1d). Vulkan twin of the
 *  CUDA and SYCL integer_moment implementations.
 *
 *  Key difference from moment_vulkan.c: accumulator type is uint64_t
 *  (unsigned) to match the CUDA twin's uint64 atomic counters exactly.
 *  The signed/unsigned distinction is numerically irrelevant for the
 *  range at hand (max 2nd-moment sum 7.2e16 << 2^63) but makes the
 *  cross-backend bit-exactness relationship explicit.
 *
 *  Pattern reference: moment_vulkan.c (PR #133, ADR-0182) and
 *  integer_moment_cuda.c — same single-dispatch + 4-slot atomic
 *  uint64 reduction shape; host divides by w*h and emits the four
 *  float_moment_{ref,dis}{1st,2nd} scores.
 *
 *  Submit-pool migration: ADR-0354 (PR-C). Pre-allocated
 *  VmafVulkanKernelSubmitPool + pre-allocated descriptor set written
 *  once at init(). All 3 SSBO bindings (ref_in, dis_in, sums) are
...
#include "integer_moment_spv.h" /* generated SPIR-V byte array */

#define INTEGER_MOMENT_WG_X 16
#define INTEGER_MOMENT_WG_Y 8
...
typedef struct {
    ...
    /* 4-slot uint64 sum buffer: [ref1st, dis1st, ref2nd, dis2nd].
     * uint64 matches the CUDA twin's unsigned atomic counters. */
    VmafVulkanBuffer *sums;

    VmafDictionary *feature_name_dict;
} IntegerMomentVulkanState;
...
} IntegerMomentPushConsts;

static int create_pipeline(IntegerMomentVulkanState *s)
{
    struct {
        int32_t width;
        int32_t height;
    ...
    const VmafVulkanKernelPipelineDesc desc = {
        ...
        .push_constant_size = (uint32_t)sizeof(IntegerMomentPushConsts),
        .spv_bytes = integer_moment_spv,
        .spv_size = integer_moment_spv_size,
...
static int alloc_buffers(IntegerMomentVulkanState *s)
{
    size_t bytes_per_pixel = (s->bpc <= 8) ? 1U : 2U;
    ...
    /* 4 uint64 slots: ref1st / dis1st / ref2nd / dis2nd. */
    err = vmaf_vulkan_buffer_alloc_readback(s->ctx, &s->sums, 4U * sizeof(uint64_t));
    return err;
}
 ...
 * (T-GPU-OPT-VK-4 / ADR-0256 / ADR-0354.) */
static int write_descriptor_set(IntegerMomentVulkanState *s, VkDescriptorSet set)
{
...
{
    (void)pix_fmt;
    IntegerMomentVulkanState *s = fex->priv;
    s->width = w;
    s->height = h;
    s->bpc = bpc;
    ...
    if (s->ctx) {
        ...
        if (err) {
            vmaf_log(VMAF_LOG_LEVEL_ERROR,
                     "integer_moment_vulkan: cannot create Vulkan context (%d)\n", err);
            return err;
        }
        s->owns_ctx = 1;
    }
...
static int upload_plane(IntegerMomentVulkanState *s, VmafVulkanBuffer *dst_buf, VmafPicture *pic)
{
                   ...
                   VmafFeatureCollector *feature_collector)
{
    ...
    /* Zero the 4-slot uint64 sums buffer before each dispatch. */
    memset(vmaf_vulkan_buffer_host(s->sums), 0, 4U * sizeof(uint64_t));
    ...
    uint32_t gx = (s->width + INTEGER_MOMENT_WG_X - 1U) / INTEGER_MOMENT_WG_X;
    uint32_t gy = (s->height + INTEGER_MOMENT_WG_Y - 1U) / INTEGER_MOMENT_WG_Y;
    IntegerMomentPushConsts pc = {
        .width = s->width,
        .height = s->height,
        .bpc = s->bpc,
        .num_workgroups_x = gx,
    ...
    const uint64_t *sums = vmaf_vulkan_buffer_host(s->sums);
...
VmafFeatureExtractor vmaf_fex_integer_moment_vulkan = {
    .name = "integer_moment_vulkan",
    .init = init,
    .extract = extract,
    .close = close_fex,
    .options = options,
    .priv_size = sizeof(IntegerMomentVulkanState),
    .flags = VMAF_FEATURE_EXTRACTOR_VULKAN,
    .provided_features = provided_features,
    /* 1 dispatch/frame, reduction-dominated. AUTO + 1080p area
     * matches moment's profile (ADR-0181 / ADR-0182). */
    .chars =
        {
