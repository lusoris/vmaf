 ...
 *
 *  Float ANSNR feature extractor on the Metal backend — T8-1 batch-2
 *  scaffold (ADR-0361).
 *
 *  Mirrors `libvmaf/src/feature/hip/float_ansnr_hip.c` call-graph-for-call-graph.
 ...
 *  the feature engine reports "float_ansnr_metal extractor found but its
 *  runtime is not implemented" rather than "float_ansnr_metal extractor
 *  not found".
 *
 *  Provided feature: `float_ansnr`.
 *  No temporal flag — float ANSNR is per-frame, no previous-frame carry.
 *  Algorithm parity: per-frame double-precision MSE accumulation divided
 *  by (w * h * peak^2), converted to dB on the host.  Runtime PR
...
typedef struct FloatAnsnrStateMetal {
     ...
     * and the MSE-accumulator buffer slot are managed by
...
{
    (void)pix_fmt;
    FloatAnsnrStateMetal *s = fex->priv;
    ...
    /* One double-precision MSE accumulator for the Y plane. */
                             ...
                             VmafFeatureCollector *feature_collector)
{
    ...
    /* Score emission (MSE -> dB conversion) lands with the runtime PR. */
    return -ENOSYS;
}
...
static int flush_fex_metal(VmafFeatureExtractor *fex, VmafFeatureCollector *feature_collector)
{
    (void)fex;
    (void)feature_collector;
    /* float_ansnr has no tail frame — return 1 (done) immediately. */
    return 1;
}
 ...
 * vmaf_fex_float_ansnr_metal;` in feature_extractor.c's
 * feature_extractor_list[]. Same pattern as every CUDA / SYCL /
 * Vulkan / HIP feature extractor (e.g. `vmaf_fex_float_ansnr_hip`). */
...
VmafFeatureExtractor vmaf_fex_float_ansnr_metal = {
    ...
    /* No TEMPORAL flag: float_ansnr is per-frame, no previous-frame carry. */
    .flags = 0,
    .chars =
        {
