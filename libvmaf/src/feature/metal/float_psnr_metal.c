 ...
 *
 *  Float PSNR feature extractor on the Metal backend — T8-1 batch-2
 *  scaffold (ADR-0361).
 *
 *  Mirrors `libvmaf/src/feature/hip/float_psnr_hip.c` call-graph-for-call-graph.
 ...
 *  the feature engine reports "float_psnr_metal extractor found but its
 *  runtime is not implemented" rather than "float_psnr_metal extractor
 *  not found".
 *
 *  Provided feature: `float_psnr`.
 *  No temporal flag — float PSNR is per-frame, no previous-frame carry.
...
{
    (void)pix_fmt;
    FloatPsnrStateMetal *s = fex->priv;
...
static int flush_fex_metal(VmafFeatureExtractor *fex, VmafFeatureCollector *feature_collector)
{
    (void)fex;
    (void)feature_collector;
    /* float_psnr has no tail frame — return 1 (done) immediately. */
    return 1;
}
 ...
 * vmaf_fex_float_psnr_metal;` in feature_extractor.c's
 * feature_extractor_list[]. Same pattern as every CUDA / SYCL /
 * Vulkan / HIP feature extractor (e.g. `vmaf_fex_float_psnr_hip`). */
...
VmafFeatureExtractor vmaf_fex_float_psnr_metal = {
    ...
    /* No TEMPORAL flag: float_psnr is per-frame, no previous-frame carry. */
    .flags = 0,
    .chars =
        {
