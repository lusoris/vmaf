...
feature/arm64/
  ...
  moment_sve2.{c,h}         # SVE2 path for float_moment (ADR-0445) — VLA f32→f64 pattern
...
| **float_moment NEON + SVE2** (ADR-0445) | `moment_neon.c` + `moment_sve2.c` + `../x86/moment_avx2.c` + scalar `../moment.h`. The SVE2 path uses the VLA f32→f64 widening pattern (step by `svcntd()`, not `svcntw()`). The Darwin opt-out (ADR-0419) prevents the SVE2 path from running on Apple Silicon even if the build compiles it. |
...
`ssimulacra2_sve2.c` was the first SVE2 consumer in the fork; `moment_sve2.c` (ADR-0445) is the second. It is
...
### float_moment SVE2 invariants (ADR-0445)
