...
int psnr_constants(const char *fmt, double *peak, double *psnr_max)
{
    if (!strcmp(fmt, "yuv420p") || !strcmp(fmt, "yuv422p") || !strcmp(fmt, "yuv444p")) {
        ...
        *peak = 255.0;
        *psnr_max = 60.0;
    } else if (!strcmp(fmt, "yuv420p10le") || !strcmp(fmt, "yuv422p10le") ||
               !strcmp(fmt, "yuv444p10le")) {
        // 10 bit gets normalized to 8 bit, peak is 1023 / 4.0 = 255.75
        // max psnr 72.0 for 10-bit per Ioannis
        *peak = 255.75;
        *psnr_max = 72.0;
    } else if (!strcmp(fmt, "yuv420p12le") || !strcmp(fmt, "yuv422p12le") ||
               !strcmp(fmt, "yuv444p12le")) {
        // 12 bit gets normalized to 8 bit, peak is (2^12 - 1) / 16.0 = 255.9375
        // max psnr 84.0 for 12-bit
        *peak = 255.9375;
        *psnr_max = 84.0;
    } else if (!strcmp(fmt, "yuv420p16le") || !strcmp(fmt, "yuv422p16le") ||
               !strcmp(fmt, "yuv444p16le")) {
