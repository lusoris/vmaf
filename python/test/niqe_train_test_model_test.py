...
from vmaf.tools.misc import empty_object, import_python_file
...
class NiqeTrainTestModelTest(unittest.TestCase):
    ...
    def test_get_xs_from_results(self):
        ...
        self.assertAlmostEqual(xs["NIQE_noref_feature_N11_scores"][0][3], -0.016672410493636325)
    ...
    def test_train(self):
        ...
        self.assertAlmostEqual(
            np.mean(model.model_dict["model"]["mu"]), 0.58721456594247923, places=4
        )
        self.assertAlmostEqual(
            np.mean(model.model_dict["model"]["cov"]), 0.0062869078795764156, places=4
        )
