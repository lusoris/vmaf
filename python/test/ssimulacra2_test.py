...
Tolerance is `places=3` (1e-3) rather than the tighter `places=4`:
the phase-1 `cbrtf` / phase-3 `powf` per-lane scalar libm calls + the
x86 SIMD TUs without `-ffp-contract=off` allow small host-dependent
drift (libm polynomials + FMA fusion decisions vary across glibc
versions and GCC / clang backends). Observed drift in CI is ~2e-4
per-frame; 1e-3 gives comfortable headroom while still flagging any
material regression.
"""
