...
class InterpolationUtils(object):
    ...
    def interpolateRateFromMetric(cls, rdPointsList, arrayDistInLogScale):
        ...
        for distInLogScale in arrayDistInLogScale:
            ...
            rate = max(
                rate, log_rate[segmentOfInterest]
            )  # make sure interpolated rate is within bounds
    ...
    @classmethod
    def computeParamsForSegments(
        cls, rdPointsList, log_rate, log_dist, H, delta, d, c, b, convertRateUsingLogarithm
    ):
        N = len(rdPointsList)
